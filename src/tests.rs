...
fn test_list_filter_by_state() {
    ...
    let loaded = load_tasks(&mut filelock.file).unwrap();
    let todo_tasks: Vec<&Task> = loaded.tasks.iter().filter(|t| t.state == "todo").collect();
...
fn test_list_filter_by_epic() {
    ...
    let loaded = load_tasks(&mut filelock.file).unwrap();
    let planning_tasks: Vec<&Task> = loaded
        .tasks
        .iter()
...
fn test_edit_state_success() {
    ...
    let mut loaded = load_tasks(&mut filelock.file).unwrap();
    loaded.tasks[0].state = "in-progress".to_string();
    save_tasks(&mut filelock.file, &loaded).unwrap();

    // Verify state changed
    let reloaded = load_tasks(&mut filelock.file).unwrap();
...
fn test_claim_task_from_todo() {
    ...
    let mut loaded = load_tasks(&mut filelock.file).unwrap();
    let task = loaded.tasks.iter_mut().find(|t| t.state == "todo").unwrap();
...
fn test_claim_with_state_filter() {
    ...
    let loaded = load_tasks(&mut filelock.file).unwrap();
    let task = loaded.tasks.iter().find(|t| t.state == "todo");
    assert!(task.is_some());
...
fn test_claim_dependency_blocking() {
    ...
    let loaded = load_tasks(&mut filelock.file).unwrap();
    let unblocked = loaded.tasks.iter().find(|t| {
        t.state == "todo"
